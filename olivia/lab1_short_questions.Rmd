...
> The second shelf from the bottom generally has much higher suger contents. The second shelf is great real-estate for products, especially ones marketed towards children who are generally shorter than adults. This placement could be used to get kids to ask their parents for more sugary (expensive) cereal. It also appears that sugars with higher sodium tend to be on the first shelf. Cereals with higher fat content are also on shelf two, but the difference is smaller than what we see with sugar. Both fat and sodium are less extreme between shelves.
...
> Similar to what we saw in the box plots, there seems to be more high-sugar content cereals on the 2nd shelf and more high-sodium cereals on the first shelf.
...
> The model placed apple jacks on the 2nd shelf, which is promising because apple jacks are very sugary.
...
drinks2 <- data.frame(
  ...
  age = drinks$age,
  high_rosn = drinks$rosn > summary(drinks2$rosn)[4],
  low_rosn = drinks$rosn < summary(drinks2$rosn)[4]
)
...
alc.mod.pois2 <- glm(numall ~ rosn:nrel + rosn + nrel, data = drinks2, family=poisson(link = "log"))

alc.mod.pois3 <- glm(numall~ high_rosn:nrel + low_rosn:nrel, data=drinks2, family=poisson(link = "log"))

summary(alc.mod.pois2)


plot(y = drinks2$numall, x = drinks2$nrel)
```
